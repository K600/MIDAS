...
char code git_revision_main[] = GIT_REVISION;
...
#define A2HEX(x) (x >= '0' && x <= '9' ? x - '0' : (x >= 'a' && x <= 'f' ? x - 'a' + 10 : 0))

unsigned short git_revision_int()
{
	unsigned short xdata r;
	char *p;
	
	if (strrchr(git_revision_main, '-'))
		p = strrchr(git_revision_main, '-')+2;
	else
		return 0;
	
	r = A2HEX(p[0])<<12 | A2HEX(p[1])<<8 | A2HEX(p[2])<<4 | A2HEX(p[3]);
	return r;
}
...
void setup(void)
{
   ...
   /* store revision */
   sys_info.revision = git_revision_int();
   if ((flags & (1 << 1)) == 0) {
...
void interprete(void) 
{
   ...
   case CMD_GET_INFO:
      ...
      send_byte(*(((unsigned char *) &sys_info.revision) + 0), &crc);   // send revision
      send_byte(*(((unsigned char *) &sys_info.revision) + 1), &crc);
...
receive_cmd:
      ...
      if (cmd == CMD_PING16 || cmd == CMD_ADDR_NODE16) {
...
unsigned char code BLOCK_F000_REMAINDER[0x996] _at_ 0xF669; // for small model
//unsigned char code BLOCK_F000_REMAINDER[0x996] _at_ 0xF669; // for large model
