...
float xdata user_data[N_PORT*8*N_VAR_CH];
...
MSCB_INFO_VAR xdata vars[N_PORT*8*N_VAR_CH];

unsigned char xdata update_data[N_PORT*8*N_VAR_CH];
...
float xdata value;
   ...
   if (master) {
      ...
      for (index=0 ; index<N_PORT*8*N_VAR_CH ; index++) {
