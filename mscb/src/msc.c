...
void print_help()
{
   ...
   puts("read <i1>-<i2> [r] [a]     Read range of node variables [repeat]  [all]");
   ...
   puts("upload [slot] <hex-file> [debug]  Upload new firmware to node in [slot] [with debug info]");
   puts("memrd <slot> <adr> <count> Read from memory in <slot>");
   puts("memwr <slot> <adr> <value> Write to memory in <slot>");
   ...
   puts("verify [adr] <hex-file>    Compare current firmware in [adr] with file");
   ...
   puts("log [c]                    Read log / clear Log [c]");
...
{
   int i, j, index, status, ivar, addr;
...
{
   int i, j, status, size, nparam, current_addr, current_group, reference_addr,
      first, last, broadcast, read_all, repeat, index, idx1, idx2, wait = 0, n_found, slot;
   int ping_addr[0x10000];
   unsigned short addr;
   unsigned int data, uptime, mem_addr;
   unsigned char c;
   float value;
   char str[256], line[256], dbuf[100 * 1024], param[10][100], *pc, lib[32],
       prot[32], *pdata, prompt[256], filename[256];
   FILE *cmd_file = NULL;
   MSCB_INFO info;
   ...
   do {
         ...
         do {
            ...
            for (i = -1; i < 0x10000; i++) {
               ...
               if (status == MSCB_SUCCESS) {
                  ...
                  if (status == MSCB_SUCCESS) {
                     printf
                         ("Found node \"%s\", NA %d (0x%04X), GA %d (0x%04X), Rev. 0x%04X      \n",
      ...
      /* info ---------- */
         ...
         if (current_addr < 0)
            ...
            if (status == MSCB_CRC_ERROR)
               ...
               // check if it's an SCS3000 or not
               ...
               {
                  printf("Revision          : V %d.%d.%d\n", info.revision>>8, (info.revision>>4) & 0xF, info.revision & 0xF );
               }
               ...
               // print variables for SCS3000
               if( info.bootBank != 0 )
               {
                  printf("Pin/Ext WD Resets : %d\n", info.pinExt_resets);
                  printf("SW-resets         : %d\n", info.SW_resets );
                  printf("Int. WD resets    : %d\n", info.int_WD_resets );
                  printf("Boot-Bank         : %d\n", info.bootBank );
                  printf("Silicon Revision  : %c\n", info.siliconRevison );
                  printf("System load       : %4.2f%%\n", info.systemLoad );
                  printf("Peak system load  : %4.2f%%\n", info.peakSystemLoad );
                  printf("PCB Temp          : %4.2f%cC\n",info.pcbTemp, 167 );   // 167 is ° symbol
                  printf("Supply 1.8V       : %4.2fV\n",info.Supply1V8 );
                  printf("Supply 3.3V       : %4.2fV\n",info.Supply3V3 );
                  printf("Supply 5.0V       : %4.2fV\n",info.Supply5V0 );
                  printf("Supply 24.0V      : %4.2fV\n",info.Supply24V0 );
                  printf("Supply 5V Ext.    : %4.2fV\n",info.Supply5V0Ext );
                  printf("Supply 24V Ext.   : %4.2fV\n",info.Supply24V0Ext );
                  printf("Supply current    : %4.3fA\n",info.Supply24V0Current );
                  printf("Battery voltage   : %4.2fV\n",info.Vbat );
               }
         ...
         if (current_addr < 0 && current_group < 0)
            ...
            if (!param[1][0] || !param[2][0])
               ...
               for (index=idx1 ; index<=idx2 ; index++) {
                 status = mscb_info_variable(fd, (unsigned short) addr, (unsigned char) index,
                                    &info_var);
      ...
      /* read ---------- */
         ...
         if (current_addr < 0)
            ...
            if (!param[1][0] || param[1][0] == 'a' || param[1][0] == 'r') {
               first = 0;
               last = 256;
            } else if(strchr(param[1], '-')) {
               first = atoi(param[1]);
               last = atoi(strchr(param[1], '-') + 1);
            } else {
            ...
            for (i = first; i <= last; i++) {
               status = mscb_info_variable(fd, (unsigned short) current_addr, (unsigned char) i, &info_var_array[i]);
               if (status == MSCB_NO_VAR) {
                  if (first == last)
                     printf("Node has no variable #%d\n", first);
                  break;
            ...
            if (n_found) {
               do {
                  if (first == last) { // read single variable
                     i = first;
                     if ((info_var_array[i].flags & MSCBF_HIDDEN) == 0 || read_all) {
                        size = info_var_array[i].width;
                        memset(dbuf, 0, sizeof(dbuf));
                        status = mscb_read(fd, (unsigned short) current_addr, (unsigned char) i, dbuf, &size);
                        
                        if (status == MSCB_SUCCESS)
                           print_channel(i, &info_var_array[i], dbuf, first != last);
                     }
                  } else { // read multiple variables
                     last = first + n_found - 1;
                     size = 1400;
                     status = mscb_read_range(fd, (unsigned short) current_addr, (unsigned char) first, (unsigned char) last, dbuf, &size);
                     
                     if( status == MSCB_CRC_ERROR )
                        puts("CRC Error\n");
                     else if( status == MSCB_TIMEOUT )
                        puts("Timeout receivig acknowledge\n");
                     else if( status != MSCB_SUCCESS )
                        puts("Reading not successful!\n");
                     else {
                        pdata = dbuf;
                        for (i = first; i <= last; i++) {
                           size = info_var_array[i].width;
      ...
      /* terminal ---------- */
         ...
         /* find first dataless variable */
         memset(info_var_array, 0, sizeof(info_var_array));
         for (chn=0; chn<255; chn++) {
            status = mscb_info_variable(fd, (unsigned short) current_addr, (unsigned char)chn, &info_var_array[chn]);
            if (status == MSCB_NO_VAR)
               break;
            if (info_var_array[chn].flags & MSCBF_DATALESS)
               break;
         }

         if (status == MSCB_NO_VAR) {
            
            puts("Node does not support terminal mode.\n");
            ...
            do {
               if ((c = ss_getchar(0)) != 0) {
                  ...
                  putchar(c);
                  fflush(stdout);
                  status = mscb_write(fd, (unsigned short) current_addr, chn, &c, 1);
                  if (status != MSCB_SUCCESS) {
                     printf("\nError: %d\n", status);
                     break;
                  }
                  ...
                  if (c == '\r') {
                     putchar('\n');
                     c = '\n';
                     mscb_write(fd, (unsigned short) current_addr, chn, &c, 1);
                  }
               ...
               memset(line, 0, sizeof(line));
               size = sizeof(line);
               mscb_read(fd, (unsigned short) current_addr, chn, line, &size);
               if (size > 0) {
                  fputs(line, stdout);
                  fflush(stdout);
               }
      ...
      /* upload ---------- */
         ...
         if (isdigit(param[1][0])) {
            // address given: upload to WaveDAQ slot
            if (param[2][0] == 0) {
               printf("Enter name of file: ");
               fgets(filename, sizeof(filename), stdin);
            } else
               strcpy(filename, param[2]);
            
            if (str[strlen(filename) - 1] == '\n')
               str[strlen(filename) - 1] = 0;

            i = 0;
            if (match(param[2], "debug") ||
                match(param[3], "deubg"))
               i = 1;
            
            status = mscb_upload(fd, (unsigned short) current_addr, (unsigned short) atoi(param[1]), filename, i | 0x04);
            if (status == MSCB_NOT_FOUND)
               printf("File \"%s\" not found\n", filename);
            else if (status == MSCB_FORMAT_ERROR)
            ...
            if (current_addr < 0)
               ...
               if (param[1][0] == 0) {
                  ...
                  strcpy(filename, param[1]);
               
               if (str[strlen(filename) - 1] == '\n')
                  str[strlen(filename) - 1] = 0;
               
               status = mscb_upload(fd, (unsigned short) current_addr, 0, filename, param[2][0] == 'd');
               if (status == MSCB_NOT_FOUND)
                  printf("File \"%s\" not found\n", filename);
               else if (status == MSCB_FORMAT_ERROR)
                  printf("Syntax error in file \"%s\"\n", filename);
      ...
      /* mupload ---------- */
         ...
         if (last == 0)
            ...
            for (i = first; i <= last; i++) {
               printf("Node%d: ", i);
               status = mscb_upload(fd, (unsigned short) i, 0, filename, 0);
      ...
      /* verify ---------- */
         ...
         if (isdigit(param[1][0])) {
            // address given: verify WaveDAQ slot
            if (param[2][0] == 0) {
               printf("Enter name of file: ");
               fgets(filename, sizeof(filename), stdin);
            } else
            ...
            status = mscb_upload(fd, (unsigned short)current_addr, (short) atoi(param[1]), filename,
                                 0x02 | 0x04);
            if (status == MSCB_NOT_FOUND)
               printf("File \"%s\" not found\n", filename);
            else if (status == MSCB_FORMAT_ERROR)
            ...
            if (current_addr < 0)
               ...
               status = mscb_upload(fd, (unsigned short) current_addr, 0, filename, 0x02);
               if (status == MSCB_NOT_FOUND)
                  printf("File \"%s\" not found\n", filename);
               else if (status == MSCB_FORMAT_ERROR)
                  printf("Syntax error in file \"%s\"\n", filename);
      ...
      /* memory write ---------- */
         ...
         if (current_addr < 0 && current_group < 0 && !broadcast)
            ...
            if (param[2][1] == 'x')
               sscanf(param[2]+2, "%X", &mem_addr);
            else
               mem_addr = atoi(param[2]);
            
            mem_addr |= MSCB_BASE_RAM;
            
            if (param[3][1] == 'x') {
               sscanf(param[3]+2, "%X", &data);
               if (strlen(param[3]) == 4)
                  size = 1;
               else if (strlen(param[3]) == 6) {
                  size = 2;
                  WORD_SWAP(&data);
               } else {
                  size = 4;
            ...
            mscb_write_mem(fd, (unsigned short) current_addr, slot, mem_addr, &data, size);
         }
      }

      /* memory read ---------- */
      else if (match(param[0], "memrd")) {
         if (current_addr < 0 && current_group < 0 && !broadcast)
            printf("You must first address node (s)\n");
         else {
            slot = atoi(param[1]);
            ...
            if (param[3][1] == 'x')
               sscanf(param[3]+2, "%X", &size);
            else
               size = atoi(param[3]);
            
            mscb_read_mem(fd, (unsigned short) current_addr, slot, mem_addr, &dbuf, size);
            
            /* print result */
            printf("0x%02X: ", mem_addr);
            for (i=0 ; i<size ; i++)
      ...
      else if (match(param[0], "log")) {
         if (broadcast)
            printf("Log command not allowed in broadcast mode!\n");
         else if( current_group > 0)
            printf("Log command not allowed in group address mode!\n"); 
         if (current_addr < 0)
            printf("You must first address a single node\n");
         else {
            
            // check first if its an SCS3000
            memset( &info, 0, sizeof(info) );
            status = mscb_info(fd, (unsigned short) current_addr, &info);
            if (status == MSCB_CRC_ERROR)
               puts("CRC Error");
            else if (status != MSCB_SUCCESS)
               puts("node does not respond.");
            else if( info.bootBank == 0)
               puts("Command not supported. Only SCS-3000 supports log functionality!");
            else {
               if (param[1][0] == 'c')
                  // clear log
                  status = mscb_ClearLog(fd, current_addr );
               else {
                  // +3 becasue of ACK and datalength of answer
                  char* LogMsg = &dbuf[3];
                  
                  printf("\r\n");
                  printf("Modul    Line    Error    Opt. Val      Time                               Message\r\n");
                  
                  // read log
                  do {
                     status = mscb_ReadLog( fd, current_addr, dbuf, sizeof(dbuf) );
                     
                     if( status == MSCB_SUCCESS )
                        printf("%s", LogMsg);
                     
                  } while (status == MSCB_SUCCESS && *LogMsg != '\0');
               }
...
int main(int argc, char *argv[])
{
   ...
   unsigned short adr;
   char cmd[256], device[256], str[256], password[256], ip[256];
   int debug;
   ...
   if (fd < 0) {
      if (fd == EMSCB_COMM_ERROR) {
      ...
      } else if (fd == EMSCB_PROTOCOL_VERSION) {
         ...
         printf("Cannot connect to device \"%s\" (%s)\n", device, ip);
      else
   ...
   printf("Connected to submaster %s (%s)\n", device, ip);
